        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div style="background: linear-gradient(135deg, #2563eb 0%, #4f46e5 100%); border-radius: 16px; padding: 32px; margin-bottom: 24px; color: white;">
                ...
                <p style="color: #dbeafe; font-size: 1rem;">
                    @if(Auth::user()->is_admin)
                        Beheer de website en bekijk het overzicht.
                    @else
            ...
            @if(Auth::user()->is_admin)
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 16px; margin-bottom: 32px;">
                    <a href="{{ route('admin.appointments.index') }}" style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
                        <div style="font-size: 2rem; font-weight: 700; color: #f59e0b;">{{ $stats['pending_appointments'] }}</div>
                        <div style="font-size: 0.875rem; color: #6b7280;">Wachtende afspraken</div>
                    </a>
                    <a href="{{ route('admin.contact.index') }}" style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
                        <div style="font-size: 2rem; font-weight: 700; color: #ef4444;">{{ $stats['unread_messages'] }}</div>
                        <div style="font-size: 0.875rem; color: #6b7280;">Ongelezen berichten</div>
                    </a>
                    <a href="{{ route('admin.glasses.index') }}" style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
                        <div style="font-size: 2rem; font-weight: 700; color: #16a34a;">{{ $stats['total_glasses'] }}</div>
                        <div style="font-size: 0.875rem; color: #6b7280;">Brillen in catalogus</div>
                    </a>
                    <a href="{{ route('admin.users.index') }}" style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
                        <div style="font-size: 2rem; font-weight: 700; color: #2563eb;">{{ $stats['total_users'] }}</div>
                        <div style="font-size: 0.875rem; color: #6b7280;">Geregistreerde klanten</div>
                    </a>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 24px;">
                    ...
                    <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); overflow: hidden;">
                        <div style="padding: 20px 24px; border-bottom: 1px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center;">
                            <h3 style="font-size: 1.125rem; font-weight: 600; color: #1f2937; margin: 0;">Wachtende afspraken</h3>
                            <a href="{{ route('admin.appointments.index') }}" style="font-size: 0.875rem; color: #2563eb; text-decoration: none;">Alles bekijken →</a>
                        </div>
                            ...
                            @forelse($pendingAppointments as $appointment)
                                <div style="padding: 12px 0; border-bottom: 1px solid #f3f4f6;">
                                    <div style="display: flex; justify-content: space-between; align-items: start;">
                                        <div>
                                            <div style="font-weight: 500; color: #1f2937;">{{ $appointment->user->name }}</div>
                                            <div style="font-size: 0.875rem; color: #6b7280;">{{ $appointment->reason }}</div>
                                        </div>
                                        <div style="text-align: right;">
                                            <div style="font-size: 0.875rem; font-weight: 500; color: #1f2937;">{{ $appointment->date->format('d/m/Y') }}</div>
                                            <div style="font-size: 0.75rem; color: #6b7280;">{{ $appointment->time_slot }}</div>
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <p style="color: #6b7280; text-align: center; padding: 24px 0;">Geen wachtende afspraken</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div style="background: white; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.07); overflow: hidden;">
                        <div style="padding: 20px 24px; border-bottom: 1px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center;">
                            <h3 style="font-size: 1.125rem; font-weight: 600; color: #1f2937; margin: 0;">Ongelezen berichten</h3>
                            <a href="{{ route('admin.contact.index') }}" style="font-size: 0.875rem; color: #2563eb; text-decoration: none;">Alles bekijken →</a>
                        </div>
                        <div style="padding: 16px 24px;">
                            @forelse($unreadMessages as $message)
                                <a href="{{ route('admin.contact.show', $message) }}" style="display: block; padding: 12px 0; border-bottom: 1px solid #f3f4f6; text-decoration: none;">
                                    <div style="font-weight: 500; color: #1f2937;">{{ $message->name }}</div>
                                    <div style="font-size: 0.875rem; color: #6b7280;">{{ $message->subject }}</div>
                                    <div style="font-size: 0.75rem; color: #9ca3af; margin-top: 4px;">{{ $message->created_at->diffForHumans() }}</div>
                                </a>
                            @empty
                                <p style="color: #6b7280; text-align: center; padding: 24px 0;">Geen ongelezen berichten</p>
                            @endforelse
                        </div>
                    </div>
                </div>
